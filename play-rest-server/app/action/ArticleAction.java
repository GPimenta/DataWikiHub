...
public class ArticleAction extends play.mvc.Action.Simple {
    ...
    @Singleton
    @Inject
    public ArticleAction(MetricRegistry metrics, ClassLoaderExecutionContext ec, Futures futures) {
        this.ec = ec;
        this.futures = futures;
        this.requestsMeter = metrics.meter("requestsMeter");
        this.responsesTimer = metrics.timer(name(Article.class, "responsesTimer"));
    }
    ...
    public CompletionStage<Result> call(Http.Request request) {
        ...
        if (request.accepts("application/json")) {
            ...
            return futures.timeout(doCall(request), 1L, TimeUnit.SECONDS).exceptionally(e -> {
                return (status(GATEWAY_TIMEOUT, views.html.timeout.render()));
